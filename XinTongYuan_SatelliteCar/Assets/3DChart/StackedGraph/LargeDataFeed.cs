...
public partial class LargeDataFeed : MonoBehaviour, IComparer<DoubleVector2>
{
    ...
    const double scrollBuffer = 0.2;
    public void Update()
    {
        if (mData.Count > 0 && graph != null)
        {
            double minScroll = mData[0].x - scrollBuffer;
            double maxScroll = mData[mData.Count - 1].x - graph.DataSource.HorizontalViewSize + scrollBuffer;

            // 注意不要让 maxScroll 小于 minScroll
            if (maxScroll < minScroll)
                maxScroll = minScroll;

            if (graph.HorizontalScrolling < minScroll)
                graph.HorizontalScrolling = minScroll;
